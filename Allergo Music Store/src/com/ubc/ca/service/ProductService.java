...
public class ProductService {
	...
	public ArrayList<Item> searchProduct(String category, String title,
			String leadSinger, int quantity, ArrayList<Item> searchList)
		...
		if (!(leadSinger.equals("") || null == leadSinger
			...
			query = "SELECT item.upc,title,item_stock,item_category,item_price "
					+ "FROM item where item.title=? " + "and item_category=?";
			...
			query = "SELECT item.upc,title,item_stock,item_category,item_price FROM "
					+ "item,leadsinger where item.upc=leadsinger.upc "
					+ "and item.title=? " + "and leadsinger.singer_name=?";
			...
			query = "SELECT item.upc,title,item_stock,item_category,item_price"
					+ " FROM item,leadsinger where item.upc=leadsinger.upc "
	 ...
	 * @throws SQLException
	 * @throws TooManyItemsFoundException
	 * @throws NoStockException
	 */
	public Item getItem(String category, String title, String leadSinger,
			int quantity) throws ConnectException, SQLException,
			TooManyItemsFoundException, NoStockException {
		System.out.println("getItem");
		Item item = null;
		Connection con = ConnectionService.getConnection();
		String query = null;
		...
		if (!(leadSinger.equals("") || null == leadSinger
			...
			query = "SELECT item.upc,title,item_stock,item_category "
					+ "FROM item,leadsinger where item.upc=leadsinger.upc and item.item_category=? "
					+ "and item.title=? " + "and leadsinger.singer_name=?";
		...
		if (!(leadSinger.equals("") || null == leadSinger)) {
			...
			countquery = "SELECT count(*) FROM item where "
					+ "FROM item,leadsinger where item.upc=leadsinger.upc "
					+ "and leadsinger.singer_name=?";
	...
	public String saveOrder(float totalprice, String customerId,
		...
		PreparedStatement query_update = con.prepareStatement(updateQuery,
				generatedColumns);
	 ...
	 * @throws SQLException
	 */
	public int updateStock(Item item, int quantity) throws ConnectException,
			SQLException {
		Connection con = ConnectionService.getConnection();
	 ...
	 * @return
	 */
	public int updateStock(ArrayList<Item> shpcart) {
		Connection con = null;
		int i = 0;
		try {
			con = ConnectionService.getConnection();
			String query = "update item set item_stock=item_stock-? where upc=?";
			PreparedStatement query_ps = con.prepareStatement(query);

			for (Item items : shpcart) {
				query_ps.setInt(1, items.getQuantity());
				query_ps.setString(2, items.getUPC());
				i = query_ps.executeUpdate();
				System.out.println(i);
			}
			con.commit();
		} catch (ConnectException e) {
